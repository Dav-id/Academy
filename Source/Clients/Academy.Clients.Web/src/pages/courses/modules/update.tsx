...
import { getCourseModule, updateModule, UpdateCourseModuleRequest, CourseModuleResponse } from '../../../services/courses/courseModuleService';
...
export default function CourseModuleUpdatePage() {
    ...
    } = useForm<UpdateCourseModuleRequest>({
        defaultValues: {
            id: initialModule.id,
            courseId: initialModule.courseId,
            title: initialModule.title,
            description: initialModule.description,
            order: initialModule.order,
        }
    });

    // Keep form in sync if loader data changes (e.g. on fast navigation)
    useEffect(() => {
        setValue('id', initialModule.id);
        setValue('courseId', initialModule.courseId);
        setValue('title', initialModule.title);
        setValue('description', initialModule.description);
        setValue('order', initialModule.order);
    }, [initialModule, setValue]);

    const onSubmit = async (data: UpdateCourseModuleRequest) => {
        setServerError(null);
        try {
            await updateModule(tenantUrlStub!, Number(courseId), Number(courseModuleId), data);
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="mx-auto max-w-4xl">
            <div className="flex w-full flex-wrap items-end justify-between gap-4">
                <Heading>Update Course Module</Heading>
